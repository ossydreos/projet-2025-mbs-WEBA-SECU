...
class _BookingScreenState extends State<BookingScreen> with TickerProviderStateMixin {
  ...
  void _centerMapOnRoute() {
    ...
    final destination = widget.destinationCoordinates ?? LatLng(48.8584, 2.2945);
  ...
  void _togglePanel() {
    ...
    setState(() {
      ...
      _currentPanelHeight = _currentPanelHeight >= midpoint ? minHeight : maxHeight;
  ...
  Widget build(BuildContext context) {
    final screenHeight = MediaQuery.of(context).size.height;
    final safeAreaBottom = MediaQuery.of(context).padding.bottom;
    final panelExpandedHeight = screenHeight * 0.45; // 45% de l'écran (min souhaité)
    final panelCollapsedHeight = screenHeight * 0.45; // même valeur pour éviter de descendre plus bas
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      onVerticalDragStart: (_) {
                        ...
                        final minHeight = screenHeight * 0.45;
                        final maxHeight = screenHeight * 0.72;
                        _dragStartHeight = _currentPanelHeight.clamp(minHeight, maxHeight);
                      },
                      onVerticalDragUpdate: (details) {
                        ...
                        final minHeight = screenHeight * 0.45;
                        final maxHeight = screenHeight * 0.72;
                        final tentative = _currentPanelHeight - details.delta.dy;
                        double bounded = tentative;
                        if (bounded < minHeight) bounded = minHeight;
                        if (bounded > maxHeight) bounded = maxHeight;
                        setState(() {
                          _currentPanelHeight = bounded;
                          _isPanelExpanded = true;
                        });
                      },
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              height: 40,
                              decoration: BoxDecoration(
                                color: _vehicles.firstWhere((v) => v['name'] == _selectedVehicle)['color'].withOpacity(0.2),
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Icon(
                                _vehicles.firstWhere((v) => v['name'] == _selectedVehicle)['icon'],
                                color: _vehicles.firstWhere((v) => v['name'] == _selectedVehicle)['color'],
                                size: 20,
                              ),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _vehicles.firstWhere((v) => v['name'] == _selectedVehicle)['description'],
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: AppColors.textSecondary,
                                    ),
                            ...
                            Text(
                              _vehicles.firstWhere((v) => v['name'] == _selectedVehicle)['price'],
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
                                color: Colors.white,
