...
class _PendingReservationsWidgetState extends State<PendingReservationsWidget> {
  ...
  Widget _buildPendingReservationCard(Reservation reservation) {
    return FutureBuilder<Duration?>(
      future: _timeoutService.getTimeUntilTimeout(reservation.id),
      builder: (context, timeoutSnapshot) {
        final timeUntilTimeout = timeoutSnapshot.data;
        final isNearTimeout = timeUntilTimeout != null && timeUntilTimeout.inMinutes <= 5;
        final isInTimeout = timeUntilTimeout != null && timeUntilTimeout.inMinutes <= 0;
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (isNearTimeout || isInTimeout) ...[
                          const SizedBox(width: 8),
                          Icon(
                            isInTimeout ? Icons.warning : Icons.access_time,
                            color: isInTimeout ? Colors.red : Colors.orange,
                            size: 16,
                          ),
                          ...
                          Text(
                            isInTimeout 
                                ? 'TIMEOUT!' 
                                : '${timeUntilTimeout.inMinutes}min',
                            style: TextStyle(
                              color: isInTimeout ? Colors.red : Colors.orange,
                              fontWeight: FontWeight.bold,
                              fontSize: 12,
